...
const sendMonthlyAttendanceReport = async (): Promise<void> => {
  ...
  try {
    ...
    for (const user of users) {
      try {
        const attendanceData = typeof (attendanceService as any).getAttendancePercentage === 'function' 
          ? await (attendanceService as any).getAttendancePercentage(user._id.toString())
          : { totalDays: 0, presentDays: 0, attendancePercentage: 0 };
