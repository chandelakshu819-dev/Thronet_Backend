...
export const getSessionById = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthRequest).user?.id;
  const session = await timerService.getSessionById(req.params.sessionId as string, userId!);
...
export const deleteSession = asyncHandler(async (req: Request, res: Response) => {
  const userId = (req as AuthRequest).user?.id;
  await timerService.deleteSession(req.params.sessionId as string, userId!);
