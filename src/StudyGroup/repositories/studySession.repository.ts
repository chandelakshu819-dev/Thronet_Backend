...
export class StudySessionRepository extends BaseRepository<IStudySession> {
  ...
  async findBySessionId(sessionId: string): Promise<IStudySession | null> {
    try {
      return await this.model
        ...
        .lean()
        .exec() as unknown as IStudySession | null;
    } catch (error: any) {
  ...
  async getStatsSummary(userId: string, dates: any): Promise<any> {
    const sessions = await this.findAllCompleted(userId);
    const totalSessions = sessions.length;
    const totalDuration = sessions.reduce((sum: number, s: any) => sum + (s.duration || 0), 0);
    return {
      totalSessions,
      totalDuration,
      averageSessionMinutes: totalSessions ? Math.round((totalDuration * 60) / totalSessions) : 0,
      today: { sessions: 0, hours: 0 },
      thisWeek: { sessions: 0, hours: 0 },
      thisMonth: { sessions: 0, hours: 0 },
    };
  }
